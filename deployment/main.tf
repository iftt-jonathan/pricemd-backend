...
locals {
  ...
  lambda_functions = [
    { route = "POST /api/v1/procedure/search", handler = "lambdas.routes.router.search_handler" },
    { route = "GET /api/v1/procedure/{procedureId}/by-insurance/{insuranceId}", handler = "lambdas.routes.router.get_handler" }
  ]
}
